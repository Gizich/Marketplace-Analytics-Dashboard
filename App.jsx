...
// --- MOCK DATA GENERATOR ---
// Генерация правдоподобных данных за последние 365 дней
const generateHistory = (basePrice, volatility, trend) => {
  ...
  for (let i = 365; i >= 0; i--) {
    ...
    // Симуляция продаж (обратная зависимость от цены + случайность)
    const salesBase = 10000 / currentPrice; 
    const sales = Math.floor(salesBase * (0.8 + Math.random() * 0.4) * (i % 7 === 0 ? 1.5 : 1)); // Выходные продают лучше

    // Симуляция конкурентов
...
export default function MarketplaceAnalytics() {
  ...
  const [timeRange, setTimeRange] = useState('month'); // week, month, half, year
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          <Card className="p-6">
            <div className="mb-6">
              ...
              <p className="text-sm text-slate-500">Изменение средней цены продажи во времени</p>
            </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            <Card className="p-6">
              <div className="mb-6">
                <h3 className="text-lg font-bold text-slate-800">Объем продаж</h3>
                <p className="text-sm text-slate-500">Количество проданных единиц товара</p>
              </div>
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
            ...
            <Card className="p-6">
              <div className="mb-6">
                <h3 className="text-lg font-bold text-slate-800">Конкуренция</h3>
                <p className="text-sm text-slate-500">Активные продавцы данного товара</p>
              </div>
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={filteredData}>
                    ...
                    <Tooltip 
                      contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}
                      formatter={(value) => [value, 'Продавцов']}
                    />
